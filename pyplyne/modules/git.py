...
class DeployGitMixin(object):
    def deploy_git_install(self,  test = False):
        ...
        for repo in repos:
            ...
            self.logger.info("Installing %s (branch %s) in %s", uri, branch, target)
    ...
    def deploy_git_checkout(self,  test = False):
        ...
        for repo in repos:
            ...
            self.logger.info("Updating %s in %s to branch %s (%s) from %s", uri, self.target, branch, local_name, cwd)
